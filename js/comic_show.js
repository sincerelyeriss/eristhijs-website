...
function writePageTitle(div,toggleNum, char) {
  if (pgData.length >= pg) {
    ...
    document.querySelector(div).innerHTML = `<h6>${pgData[pg - 1].title}</h6>`;
    if (toggleNum) {
        //toggle whether you want to display the page number
        document.querySelector(div).innerHTML = `<h6>${pgData[pg - 1].pgNum + char + pgData[pg - 1].title}</h6>`; //char denotes a separating character between the number and the title
    }
  }
}
